/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DataColumnWhereInput } from "./DataColumnWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { DataColumnOrderByInput } from "./DataColumnOrderByInput";

@ArgsType()
class DataColumnFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DataColumnWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => DataColumnWhereInput, { nullable: true })
  @Type(() => DataColumnWhereInput)
  where?: DataColumnWhereInput;

  @ApiProperty({
    required: false,
    type: [DataColumnOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [DataColumnOrderByInput], { nullable: true })
  @Type(() => DataColumnOrderByInput)
  orderBy?: Array<DataColumnOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DataColumnFindManyArgs as DataColumnFindManyArgs };
